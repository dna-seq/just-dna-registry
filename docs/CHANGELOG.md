...
`bool(verdict)`.** The field has said *no mismatch was found* since 0.11, with vacuity carried by
...
and genes it asked about, since 0.11) and is unaffected. RM235's new `tables_unreadable` arm — a
...
- **RM242's `alphagenome check` fix** is on a pass this service does not run. **RM247 is not a
  pass: it is the enricher wheel being importable at all.** Its Atlas bindings were generated by a
  newer `protoc` than the runtime `grpcio` the floor allowed, and two modules import `atlas_client`
  at module scope, so `enrich` and `literature` — both of which `services/enrich.py` imports — died
  with it wherever the Atlas extra was installed. It never bit here because this deployment does not
  install that extra: `grpcio` is absent from our lock, and the guard's `ImportError` arm is the one
  that runs. 0.7.1 pins the pair, so a box that does add the extra gets a working import rather than
  a working guard.
